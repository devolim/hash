...
/// Minimal BIP-39 implementation (generate + validate mnemonic).
/// Replaces the abandoned `bip39` package to unblock pointycastle ^4.0.0.
class Bip39Helper {
  Bip39Helper._();

  /// Generate a BIP-39 mnemonic phrase.
  /// [strength] in bits: 128 → 12 words, 256 → 24 words.
  static String generateMnemonic({int strength = 256}) {
    assert(strength % 32 == 0 && strength >= 128 && strength <= 256);
  ...
  /// Validate a BIP-39 mnemonic (word count + checksum).
  static bool validateMnemonic(String mnemonic) {
    final words = mnemonic.toLowerCase().trim().split(RegExp(r'\s+'));
    if (![12, 15, 18, 21, 24].contains(words.length)) return false;

    // Build word→index map lazily
    final wordMap = <String, int>{};
    for (var i = 0; i < _wordlist.length; i++) {
      wordMap[_wordlist[i]] = i;
    }
    ...
    final checksumBits = totalBits ~/ 33; // ENT/32, and totalBits = ENT + ENT/32
    final entropyBits = totalBits - checksumBits;

    // Extract entropy bytes
    final entropyBytes = Uint8List(entropyBits ~/ 8);
    for (var i = 0; i < entropyBytes.length; i++) {
      entropyBytes[i] = int.parse(bitStr.substring(i * 8, i * 8 + 8), radix: 2);
    }
    ...
    final hashBits = hash.bytes
        ...
        .join();
    final expectedChecksum = hashBits.substring(0, checksumBits);
    final actualChecksum = bitStr.substring(entropyBits);
  ...
  static String _entropyToMnemonic(Uint8List entropy) {
    ...
    final checksumBits = entropy.length ~/ 4; // ENT/32 in bits
    ...
    final hashBits = hash.bytes
        .map((b) => b.toRadixString(2).padLeft(8, '0'))
        .join();
    bits.write(hashBits.substring(0, checksumBits));
