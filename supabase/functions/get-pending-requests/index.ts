...
serve(async (req) => {
  ...
  try {
    ...
    const { data: requests, error } = await supabaseAdmin
      .from('contact_requests')
      .select('id, requester_hash_id, created_at, expires_at, encrypted_message')
    ...
    if (error) {
      console.error('[GET-PENDING-REQUESTS] Query error:', error)
      return new Response(
    ...
    console.log('[GET-PENDING-REQUESTS] demandes en attente récupérées')
    ...
    console.error('[GET-PENDING-REQUESTS] Unexpected error:', error)
    return new Response(
